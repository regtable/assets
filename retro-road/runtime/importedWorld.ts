...
export class ImportedWorld {
    ...
    private binsMap = new Map<THREE.Object3D, {
        binId: string;
        binData: BinCategory;
    }>();
        ...
        for (const area of this.loadedAreas) {
            ...
            area.group.traverse((node) => {
                ...
                if (userData.role === "record_bin") {
                    ...
                    if (rawBinObj &&
                        ...
                        validParsedBin = {
                            ...
                            yearRange: Array.isArray(rawBinObj.yearRange) && rawBinObj.yearRange.length === 2 ? rawBinObj.yearRange : undefined,
                            tags: rawBinObj.tags
                        };
                    ...
                    if (!matchedBin) {
                        matchedBin = BINS_DATA.find((b) => b.id === binId || b.name.toLowerCase() === String(binId).toLowerCase());
                    }
                    if (matchedBin) {
                ...
                if (isStoreDoor || isBackroomHinge) {
                    ...
                    const openAngle = typeof userData.openAngle === "number" && Number.isFinite(userData.openAngle)
                        ? userData.openAngle
                        : Math.PI / 2;
                    let matchingClip: THREE.AnimationClip | undefined;
                    if (area.animationClips.length > 0) {
                        matchingClip = area.animationClips.find((c) => {
                            return (c.name === userData.animationClip ||
                                c.name === nodeName ||
                ...
                if (userData.role === "collider" || userData.shape === "box" || /_collider|_col/i.test(nodeName)) {
                ...
                if (userData.role === "trigger" || userData.role === "shop_volume" || /_trigger|_vol/i.test(nodeName)) {
                    if ((node as THREE.Mesh).isMesh) {
                        const mesh = node as THREE.Mesh;
                        mesh.visible = false;
                        if (!mesh.geometry.boundingBox) {
        ...
        for (const area of this.loadedAreas)
            area.group.traverse(node => {
                if (node.userData.role === 'trigger' && node.userData.action === 'door') {
                    ...
                    if (door)
                        this.doorMeshToData.set(node, door);
                }
            });
    ...
    public update(dt: number, playerPosition?: THREE.Vector3): void {
        ...
        for (const door of this.doorsMap.values()) {
            ...
            if (playerPosition && typeof autoClose === 'number' && autoClose > 0 && door.isOpen && door.pivotMesh.getWorldPosition(new THREE.Vector3()).distanceTo(playerPosition) > autoClose) {
                door.isOpen = false;
            ...
            if (Math.abs(door.currentProgress - door.targetProgress) > 0.001) {
                ...
                if (door.action && door.mixer) {
                    const currentTime = THREE.MathUtils.lerp(door.closedTime, door.openTime, door.currentProgress);
    ...
    private checkCollisionAt(pos: THREE.Vector3): boolean {
        ...
        for (const col of this.colliders) {
            ...
            if (door && typeof threshold === 'number' && door.currentProgress >= threshold)
                continue;
    ...
    public pick(ndc: THREE.Vector2, camera: THREE.Camera): PickResult {
        ...
        const validHits = intersects.filter((hit) => {
            ...
            while (curr) {
                const doorTrigger = curr === hit.object && curr.userData.role === 'trigger' && this.doorMeshToData.has(curr);
                if (!curr.visible && !doorTrigger)
                    return false;
                curr = curr.parent;
            }
            return true;
