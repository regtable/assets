...
export class AreaManifestLoader {
    ...
    public async loadManifest(manifestUrl = "https://raw.githubusercontent.com/regtable/assets/main/retro-road/areas.json", onProgress?: (progress: LoadAreasProgress) => void): Promise<LoadedAreaResult[]> {
        if (this.isCancelled)
            return [];
        if (onProgress) {
            onProgress({
        ...
        for (let i = 0; i < json.areas.length; i++) {
            ...
            const validateVector3 = (vec: any, defaultVec: [
                number,
                number,
                number
            ], name: string): [
                ...
                if (!Array.isArray(vec) ||
                    ...
                    throw new Error(`Area "${trimmedId}" has invalid ${name}: expected 3 finite numbers, got ${JSON.stringify(vec)}`);
                }
        ...
        for (const area of validAreas) {
            ...
            try {
                ...
                const errMessage = `Failed to load GLB model for area "${area.id}" from ${area.resolvedUrl}: ${err?.message || String(err)}`;
                if (onProgress) {
                    onProgress({
                        total,
                        loaded,
